...
export function fetchReply() {
  ...
  function* fetchReplySagaWorker() {
    try {
      const comments = yield call(CommentsService.getReply);
  ...
  function* createReplySagaWorker(action) {
    ...
    yield call(CommentsService.createReply, userId,commentId, content);
  ...
  export function ReplySelector(state) {
    console.log(JSON.stringify(state.reply.reply)+"reply state");
